...
def main(test_path):
    ...
    if near_duplicates:
        ...
        for a,b,s in near_duplicates:
            find_img_path = b
            break
        sup = find_img_path.index('/')
        stock_folder = find_img_path[:sup+8]
        ...
        for folder in folders1:
            ...
            for img in imgs:
                if not img.endswith('.txt'):
                    ...
                    with open(stock_folder+folder+'/'+img,'r') as f:
                        url = f.readline()
                        f.readline()
                        title = f.readline()
    ...
    # try:
    #     # 计算原数据的signatures 和 hash_buckets_list
    #     signatures,hash_buckets_list = get_pre_data(input_dir, hash_size, bands)
    #     # 加入保存的待检测图片文件，输出最近点检测结果
    #     near_duplicates = search(signatures,hash_buckets_list,hash_size,bands,threshold,test_path)
    #     # near_duplicates  是一个元组：a图片地址（就是搜索图片地址） b图片地址（找到的图片地址） 相似度s
    #     if near_duplicates:
