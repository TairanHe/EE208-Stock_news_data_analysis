...
class IndexFiles(object):
    ...
    def indexDocs(self, root, writer):
        ...
        with open('static/Acodes_names.csv', encoding='UTF-8') as f:
            lines = f.readlines()
            i = 1
            title_set = set()
            while i < 3777:
                ...
                try:
                    ...
                    for file in files:
                        ...
                        if title in title_set:
                            ...
                            title_set.add(title)
                        doc.add(Field("Acodes", number, t2))
                        doc.add(Field("stocksname", name, t2))
                        contents = get_content(root, file)
                        ...
                        doc.add(Field("start", path[0], t1))
                        doc.add(Field("sum", len(path), t1))
...
if __name__ == '__main__':
    ...
    start = datetime.now()
    try:
        IndexFiles("static/img_article/", "index")
        end = datetime.now()
        print(end - start)
    except Exception as e:
        print("Failed: ", e)
